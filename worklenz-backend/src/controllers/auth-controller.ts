...
export default class AuthController extends WorklenzControllerBase {
  ...
  public static async status_check(
  ...
  public static async checkPasswordStrength(
    req: IWorkLenzRequest,
    res: IWorkLenzResponse
  ) {
    const result = PasswordStrengthChecker.validate(
  ...
  public static verify(req: IWorkLenzRequest, res: IWorkLenzResponse) {
    ...
    if (req.user) req.user.build_v = FileConstants.getRelease();
  ...
  public static logout(req: IWorkLenzRequest, res: IWorkLenzResponse) {
    req.logout((err) => {
      if (err) {
        ...
        return res
          .status(500)
          .send(new AuthResponse(null, true, {}, "Logout failed", null));
      }
      ...
      req.session.destroy((destroyErr) => {
        ...
        res
          .status(200)
          .send(new AuthResponse(null, req.isAuthenticated(), {}, null, null));
      });
    });
  }
  ...
  public static async changePassword(
    ...
    if (data) {
      ...
      if (bcrypt.compareSync(currentPassword, data.password)) {
        ...
        await db.query(updatePasswordQ, [
          encryptedPassword,
          req.user?.id || null,
        ]);
        ...
        return res
          .status(200)
          .send(
            new ServerResponse(true, null, "Password updated successfully!")
          );
      }

      return res
        .status(200)
        .send(new ServerResponse(false, null, "Old password does not match!"));
    }
  }

  @HandleExceptions({ logWithError: "body" })
  public static async reset_password(
    req: IWorkLenzRequest,
    res: IWorkLenzResponse
  ) {
    ...
    if (!result.rowCount)
      return res
        .status(200)
        .send(new ServerResponse(false, null, "Account does not exists!"));
    ...
    if (data?.google_id) {
      return res
        .status(200)
        .send(new ServerResponse(false, null, "Password reset failed!"));
    }
    ...
    return res
      .status(200)
      .send(new ServerResponse(false, null, "Email not found!"));
  }

  @HandleExceptions({ logWithError: "body" })
  public static async verify_reset_email(
    req: IWorkLenzRequest,
    res: IWorkLenzResponse
  ) {
    ...
    if (
      bcrypt.compareSync(data.id + data.email + data.password, hashedString)
    ) {
      ...
      return res
        .status(200)
        .send(new ServerResponse(true, null, "Password updated successfully"));
    }
    return res
      .status(200)
      .send(
        new ServerResponse(false, null, "Invalid Request. Please try again.")
      );
  }
  ...
  public static async verifyCaptcha(
    ...
    try {
      ...
      return res
        .status(400)
        .send(
          new ServerResponse(false, null, "Please try again later.").withTitle(
            "Error"
          )
        );
    } catch (error) {
  ...
  @HandleExceptions({ logWithError: "body" })
  public static async googleMobileAuth(
    req: IWorkLenzRequest,
    res: IWorkLenzResponse
  ) {
    ...
    if (!idToken) {
      return res
        .status(400)
        .send(new ServerResponse(false, null, "ID token is required"));
    }
    ...
    try {
      ...
      if (!allowedClientIds.includes(profile.aud)) {
        return res
          .status(400)
          .send(new ServerResponse(false, null, "Invalid token audience"));
      }
      ...
      if (
        ...
        return res
          .status(400)
          .send(new ServerResponse(false, null, "Invalid token issuer"));
      }
      ...
      if (Date.now() >= profile.exp * 1000) {
        return res
          .status(400)
          .send(new ServerResponse(false, null, "Token expired"));
      }

      if (!profile.email_verified) {
        return res
          .status(400)
          .send(new ServerResponse(false, null, "Email not verified"));
      }
        ...
        return res
          ...
          .send(
            new ServerResponse(
              false,
              null,
              `No Google account exists for email ${profile.email}.`
            )
          );
      }
      ...
      req.login(user, (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .send(new ServerResponse(false, null, "Authentication failed"));
        }
      ...
      return res
        .status(400)
        .send(new ServerResponse(false, null, "Invalid ID token"));
    }
  }
}
