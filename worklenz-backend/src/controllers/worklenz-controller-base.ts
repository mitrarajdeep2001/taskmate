...
export default abstract class WorklenzControllerBase {
  ...
  public static createTagList(
    list: Array<{ name?: string; end?: boolean; names?: string[] }>,
    max = 4
  ) {
    let data = [...(list || [])];
    if (data.length > max) {
  ...
  protected static toPaginationOptions(
    queryParams: any,
    ...
    // Sort
    const sortField = /null|undefined/.test(queryParams.field as string)
      ? searchField
      : queryParams.field;
