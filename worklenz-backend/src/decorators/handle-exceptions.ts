...
const isValid = (options: any, key: string) =>
  Object.keys(options[key] || {}).length > 0;
const mergeWithDefaults = (options: any) => ({
  ...defaults,
  ...(options || {}),
});
...
function getConstraintResponse(constraint: string) {
  if (constraint === "[IGNORE]") return new ServerResponse(true, null);
...
function getExceptionMessage(opt: any, keys: any[]) {
  return (opt.raisedExceptions[keys[0]] || DEFAULT_ERROR_MESSAGE).replace(
    /(\{0\})/g,
    keys[1] || "",
  );
}
