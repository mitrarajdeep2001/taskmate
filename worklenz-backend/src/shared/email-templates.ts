...
export function sendWelcomeEmail(email: string, name: string) {
  let content = FileConstants.getEmailTemplate(
    IEmailTemplateType.Welcome,
  ) as string;
  if (!content) return;
...
export function sendNewSubscriberNotification(subscriberEmail: string) {
  let content = FileConstants.getEmailTemplate(
    IEmailTemplateType.NewSubscriber,
  ) as string;
  if (!content) return;
  ...
  sendEmail({
    subject: "TaskMate - New Subscriber.",
    html: content,
  });
}

export function sendJoinTeamInvitation(
  myName: string,
  teamName: string,
  teamId: string,
  userName: string,
  ...
  let content = FileConstants.getEmailTemplate(
    IEmailTemplateType.TeamMemberInvitation,
  ) as string;
  if (!content) return;
  ...
  content = content.replace("[VAR_HOSTNAME]", sanitize(FRONTEND_URL));
  content = content.replace("[VAR_BACKEND_URL]", sanitize(BACKEND_URL));
  content = content.replace("[VAR_TEAM_ID]", sanitize(teamId));
  content = content.replace("[VAR_USER_ID]", sanitize(userId));
  content = content.replace(
    "[PROJECT_ID]",
    projectId ? sanitize(projectId as string) : "",
  );

  sendEmail({
    to: [toEmail],
    subject: `${myName} has invited you to work with ${teamName} in TaskMate`,
    html: content,
  });
}

export function sendRegisterAndJoinTeamInvitation(
  myName: string,
  userName: string,
  teamName: string,
  teamId: string,
  ...
  let content = FileConstants.getEmailTemplate(
    IEmailTemplateType.UnregisteredTeamMemberInvitation,
  ) as string;
  if (!content) return;
  console.log(content, "sendRegisterAndJoinTeamInvitation");
...
export function sendResetEmail(toEmail: string, user_id: string, hash: string) {
  let content = FileConstants.getEmailTemplate(
    IEmailTemplateType.ResetPassword,
  ) as string;
  if (!content) return;
  ...
  sendEmail({
    to: [toEmail],
    subject: "Reset your password on TaskMate.",
    html: content,
  });
}

export function sendResetSuccessEmail(toEmail: string) {
  const content = FileConstants.getEmailTemplate(
    IEmailTemplateType.PasswordChange,
  ) as string;
  if (!content) return;
...
export function sendInvitationEmail(
  ...
  if (isNewMember) {
    ...
    // userNameOrId = userId
    sendRegisterAndJoinTeamInvitation(
      user?.name as string,
      userName as string,
      user?.team_name as string,
      user.team_id as string,
