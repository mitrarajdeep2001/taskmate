...
const ReportingSider = () => {
        ...
        if (item.children) {
          return {
            ...
            children: item.children.map(child => ({
              key: child.key,
              label: <Link to={`/taskmate/reporting/${child.endpoint}`}>{t(`${child.name}`)}</Link>,
            })),
          };
        }
        return {
          key: item.key,
          label: <Link to={`/taskmate/reporting/${item.endpoint}`}>{t(`${item.name}`)}</Link>,
        };
      }),
    [t]
  ...
  const activeKey = useMemo(() => {
    const afterWorklenzString = location.pathname?.split('/taskmate/reporting/')[1];
